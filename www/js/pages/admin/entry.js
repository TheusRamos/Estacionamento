...
export async function initAdminEntry(user) {
  ...
  document.getElementById('exit-payment-method')?.addEventListener('change', e => {
    document.getElementById('pix-qrcode')?.classList.toggle('d-none', e.target.value !== 'pix');
  });
...
function resetExitForm() {
  ...
  if (pm) pm.value = '';
  document.getElementById('pix-qrcode')?.classList.add('d-none');
}
